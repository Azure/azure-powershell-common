...
namespace Microsoft.Azure.Commands.Common.Authentication
{
    ...
    public partial class UserTokenAuthenticationProvider : Microsoft.Rest.ITokenProvider
    {
            ...
            ActiveDirectoryServiceSettings serviceSettings, TokenCache cache)
        {
            ...
            var account = authenticationContext.GetAccountAsync(username)
                            .ConfigureAwait(false).GetAwaiter().GetResult();
            try
            {
                var authResult = await authenticationContext.AcquireTokenSilentAsync(new string[] { "User.Read" }, account).ConfigureAwait(false);
                return
